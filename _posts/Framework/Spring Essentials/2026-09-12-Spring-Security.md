...
![Untitled]({{ site.baseurl }}{{ page.url }}/img//session.png)_개발자 도구_
...
[출자: spring.io-session-management](https://docs.spring.io/spring-security/reference/servlet/authentication/session-management.html)
...
여기서 **SessionManagementFilter**의 경우, `SessionManagementConfigurer`의 메서드를 사용하면 내부적으로 필터가 켜집니다.
...
(SessionManagementFilter는 로그인되는 순간에 세션 ID를 바꾸어 하이재킹 방지를 수행하는데, 이미 세션에 인증 정보가  있다면 검사 조차 안함.)

### 명시적 저장 + 하이재킹 방지

Spring Security는 **`AbstractAuthenticationProcessingFilter`**라는, 인증 처리를 위한 특별한 **추상 클래스**를 제공합니다.
