...
date: 2026-08-23 11:11:12 +0800
categories: [k8s, CKA]
tags: [k8s, Container, Ingress, Gateway API]
render_with_liquid: true
comments: true
image:
  path: assets/img/metaimg/nginx.png
---
...
| **HTTPRoute (6개)** | 라우팅 로직 | 호스트/경로별로 어느 Service로 보낼지, 리다이렉트, 헤더 변경 등 |

Gateway는 가장 앞단에서 하나로 정리되고, 상세 라우팅 규칙만 서비스별 HTTPRoute로 분리됩니다.

HTTPRoute는 **서비스별 라우팅용 5개 + HTTP→HTTPS 리다이렉트용 공용 1개**로 구성됩니다. 리다이렉트는 서비스마다 만들 필요 없이 Gateway와 같은 위치에 하나만 두면 됩니다.
...
별도의 `hostname` 없이 와일드카드 형태로 모든 HTTP 요청을 위 `http`라는 리스너로 받습니다. 그리고 여기에 붙는 리다이렉트용 `HTTPRoute`를 만들어주면 되는데, 이건 서비스별로 만들 필요 없이 공용 1개로 처리할 수 있습니다.
...
`RequestRedirect`에 `hostname`을 따로 지정하지 않으면 원래 요청의 호스트/경로/쿼리스트링은 그대로 유지된 채 스킴만 `https`로 바뀝니다.
