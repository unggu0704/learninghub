...
> Pod의 메모리 관리와 JVM의 구조에 대해 먼저 알아본 글 <br>
> [unggu.dev | [K8s/JVM] Node부터 JVM의 Memory 관리](https://unggu.dev/devops/container/memory/)
...
더 나아가 GC 중간에는 STW(Stop The Wolrd)가 발생하는데 Full GC는 Heap영역의 객체가 많을시, 수초~수분까지 STW가 발생하여 서비스 장애로까지 영향을 끼칠 수 있습니다.

그렇기에 무거운 명령어 보다는 현재 Pod의 RSS를 확인하여 JVM 메모리가 어떻게 분배되어 있는지 파악하는 것이 더 중요합니다.
...
![image.png]({{ site.baseurl }}{{ page.url }}/img/argocddeploy.png)
