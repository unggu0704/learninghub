...
일반적으로 메모리가 이렇게 증가하는 이유는 Heap 영역에 객체들이 많이 적재되어 있기 때문입니다.

그렇기에 이를 정리하기 위해 Full GC를 수행하는 것도 하나의 방법일 수 있습니다. 하지만 실제 운영 환경에서는 이러한 방식이 치명적인 Side Effect를 유발할 수 있습니다.
...
이로써 현재 메모리 증가의 주요 원인이 Old Gen 영역에 있음을 확인할 수 있었습니다.

> *`jcmd` 또한 Attach 과정에서 수십 Mi 정도의 추가 메모리를 사용할 수 있습니다.*
> *OOM에 근접한 상황에서는 해당 명령어 사용에도 충분한 주의가 필요합니다.*
