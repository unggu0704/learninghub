---
title: "08. [Azure/Spring] 같은 클러스터 내 Pod 통신 이슈"
author: "unggu"
date: 2025-02-15 18:32:12 +0800
...
![image]({{ site.baseurl }}{{ page.url }}/img/artice.png)

대부분의 CI/CD 파이프라인과 Azure 리소스 자원이 찍힌 상태로 아키텍쳐는 위와 같다.
위 구조로 서비스를 배포하고 서비스에 접속하니 하나의 에러와 마주쳤다.

### Front ↔ Backend 통신 에러

FrontEnd(Vue.js)와 BackEnd(Spring Boot)를 별도의 pod으로 기동 시키고 FrontEnd의 `External-IP` 를 기반으로 웹 서비스에 접속하면 아래와 같이 Axios 에러가 발생한다. 

![스크린샷 2025-02-09 오후 4.38.12.png]({{ site.baseurl }}{{ page.url }}/img/p1.png)
...
![image.png]({{ site.baseurl }}{{ page.url }}/img/p2.png)
...
Pod끼리 통신을 해야한다면 `Type` 이 `LoadBalancer` 가 아닌 `Cluster` 로 하는게 옳은 방법이라 하여 BackEnd Pod의 통신 방식을 다시 `Cluster` 방식으로 변경하였다.
...
- [`http://journeyjinni-backend.journeyjinni.svc.cluster.local:80`](http://journeyjinni-backend.journeyjinni.svc.cluster.local:80) *(Axios 에러)*
- 57.155.17.41 *(정상 호출)*
- [`http://journeyjinni-backend`](http://journeyjinni-backend/) *(Axios 에러)*

### BackEnd Endpoint의 주소를 External-IP로 설정

![스크린샷 2025-02-12 오후 8.26.14.png]({{ site.baseurl }}{{ page.url }}/img/p3.png)
...
![image.png]({{ site.baseurl }}{{ page.url }}/img/p4.png)
