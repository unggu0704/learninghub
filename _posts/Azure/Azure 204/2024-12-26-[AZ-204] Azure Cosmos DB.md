...
하나의 Azure 구독은 최대 50개의 DB계정을 보유할 수 있는데 그 밑에 Azure Cosmos DB 컨테이너 (*스케일링 성능을 위한 기본단위*) 가 존재한다. 이 컨테이너는 무제한의 프로비전과 무제한의 처리량을 가질 수 있게 된다.
...
주로 .NET SDK에서 제공하는 라이브러리로 변경 피드에서 읽은 변경사항을 쉽게 처리할 수 있게 지원하는 기능
주로 데이터를 읽고 사용자가 지정한 논리로 처리하며 병렬처리를 지원한다. 또한 임대 컨테이너를 통해 상태를 관리하고 작업 중단후에도 복구가 가능하다. 
