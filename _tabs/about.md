---
title: Introduce
icon: fas fa-info-circle
order: 4
---

# 👋 안녕하세요, 김규형입니다

<div class="d-flex align-items-center mb-4 p-4 bg-light rounded shadow-sm">
  <img src="/assets/img/favicons/me.jpeg" class="rounded-circle me-3 shadow" width="100" alt="Profile">
  <div>
    <h3 class="mb-1">김규형 (Unggu)</h3>
    <p class="text-muted mb-2">Backend Developer & DevOps Engineer</p>
    <p class="mb-0">
      <i class="fas fa-envelope text-primary"></i>
      <a href="mailto:kyuhyeong.kim@kt.com" class="text-decoration-none">kyuhung1001@gmail.com</a>
    </p>
  </div>
</div>

> Spring 기반 클라우드 네이티브 애플리케이션과 DevOps 문화에 관심이 많은 주니어 개발자입니다.  
> 알맞은 기술과 구조로 소프트웨어를 설계하고, 이를 통해 코드를 다듬어 좋은 산출물을 만드는 것을 목표로 하고 있습니다.  
> 최근에는 Kubernetes 환경에서 안정적이고 확장 가능한 시스템을 설계하고 개발하는 데 집중하고 있습니다.
{: .prompt-info }

---

## 💼 경력

<div class="position-relative ps-4 mb-5">
  <div class="position-absolute top-0 start-0 bottom-0" style="width: 2px; background: linear-gradient(to bottom, #0d6efd, #198754);"></div>
  
  <div class="mb-5 position-relative">
    <div class="card border-primary shadow-sm">
      <div class="card-header bg-primary text-white">
        <h5 class="mb-0">DevOps Engineer @ KT DS</h5>
        <small>2025.01 ~ 현재</small>
      </div>
      <div class="card-body">
        <ul class="mb-3">
          <li>Azure 클라우드 환경에서 인프라 구축 및 운영</li>
          <li>Kubernetes(AKS) 기반 대고객 서비스 운영</li>
          <li>GitOps 기반 CI/CD 파이프라인 구축 및 자동화</li>
          <li>레거시 시스템의 클라우드 마이그레이션 수행</li>
        </ul>
        <div class="alert alert-success mb-0">
          <strong>주요 성과:</strong>
          <ul class="mb-0 mt-2">
            <li>KT 대고객 서비스 Azure Migration 무중단 전환</li>
            <li>배포 시간 83% 단축 (30분 → 5분)</li>
            <li>장애 발생률 100% 감소 (연 2건 → 0건)</li>
          </ul>
        </div>
      </div>
    </div>
  </div>

  <br>

  <div class="position-relative">
    <div class="card border-success shadow-sm">
      <div class="card-header bg-success text-white">
        <h5 class="mb-0">Software Engineer @ KT DS</h5>
        <small>2024.07 ~ 현재</small>
      </div>
      <div class="card-body">
        <ul class="mb-3">
          <li>Java/Spring 기반 대고객 서비스 개발 및 운영</li>
          <li>고객 SR(Service Request) 및 VOC 대응 개발</li>
          <li>백오피스 시스템 운영 효율화 및 개선</li>
        </ul>
        <div class="alert alert-info mb-0">
          <strong>주요 성과:</strong>
          <ul class="mb-0 mt-2">
            <li>SR/VOC 대응률 100% 달성</li>
            <li>스모크 테스트 자동화로 점검 시간 70% 단축</li>
            <li>JMeter & K8s 활용 부하테스트 도구 개발</li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</div>

---

## 🎓 교육 & 자격증

### 교육 과정

<div class="row g-3 mb-4">
  <div class="col-md-6">
    <div class="card h-100 border-primary">
      <div class="card-body">
        <h6 class="card-subtitle mb-2 text-primary">KTDS University</h6>
        <p class="card-text mb-0">MSA 기반 클라우드 디자인 패턴 학습</p>
      </div>
    </div>
  </div>
  <div class="col-md-6">
    <div class="card h-100 border-success">
      <div class="card-body">
        <h6 class="card-subtitle mb-2 text-success">SSAFY</h6>
        <p class="card-text mb-0">Spring/Vue.js 기반 WEB 개발 전 과정</p>
      </div>
    </div>
  </div>
  <div class="col-md-6">
    <div class="card h-100 border-info">
      <div class="card-body">
        <h6 class="card-subtitle mb-2 text-info">NHN Academy</h6>
        <p class="card-text mb-0">Java/Servlet & 올바른 SW 설계법</p>
      </div>
    </div>
  </div>
  <div class="col-md-6">
    <div class="card h-100 border-secondary">
      <div class="card-body">
        <h6 class="card-subtitle mb-2 text-secondary">전북대학교</h6>
        <p class="card-text mb-0">컴퓨터공학 전공</p>
      </div>
    </div>
  </div>
</div>

### 보유 자격증

<div class="row g-2 mb-4">
  <div class="col-lg-4 col-md-6">
    <div class="card h-100 shadow-sm">
      <div class="card-body p-3">
        <div class="d-flex align-items-center">
          <i class="fas fa-certificate fa-2x text-info me-3"></i>
          <div>
            <h6 class="mb-0 small">Certified Kubernetes Administrator (CKA)</h6>
            <small class="text-muted">2024.08</small>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="col-lg-4 col-md-6">
    <div class="card h-100 shadow-sm">
      <div class="card-body p-3">
        <div class="d-flex align-items-center">
          <i class="fas fa-certificate fa-2x text-primary me-3"></i>
          <div>
            <h6 class="mb-0 small">Azure Developer Associate(AZ-204)</h6>
            <small class="text-muted">2025.02</small>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="col-lg-4 col-md-6">
    <div class="card h-100 shadow-sm">
      <div class="card-body p-3">
        <div class="d-flex align-items-center">
          <i class="fas fa-certificate fa-2x text-success me-3"></i>
          <div>
            <h6 class="mb-0 small">Azure Fundamentals(AZ-900)</h6>
            <small class="text-muted">2024.11</small>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="col-lg-4 col-md-6">
    <div class="card h-100 shadow-sm">
      <div class="card-body p-3">
        <div class="d-flex align-items-center">
          <i class="fas fa-database fa-2x text-warning me-3"></i>
          <div>
            <h6 class="mb-0 small">SQLD</h6>
            <small class="text-muted">2024.06</small>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="col-lg-4 col-md-6">
    <div class="card h-100 shadow-sm">
      <div class="card-body p-3">
        <div class="d-flex align-items-center">
          <i class="fas fa-code fa-2x text-danger me-3"></i>
          <div>
            <h6 class="mb-0 small">정보처리기사</h6>
            <small class="text-muted">2023.06</small>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="col-lg-4 col-md-6">
    <div class="card h-100 shadow-sm">
      <div class="card-body p-3">
        <div class="d-flex align-items-center">
          <i class="fas fa-language fa-2x text-secondary me-3"></i>
          <div>
            <h6 class="mb-0 small">TOEIC(845) / JLPT(N3)</h6>
            <small class="text-muted">2023 / 2020</small>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

---

## 🛠 기술 스택

<div class="card shadow-sm mb-4">
  <div class="card-body">
    <div class="row g-4">
      <div class="col-12">
        <h6 class="text-primary mb-3">
          <i class="fas fa-code"></i> Languaga & Framework
        </h6>
        <ul class="list-unstyled ms-3">
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Java</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Spring Framework/Spring Boot</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>RESTful API</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Swift/SwiftUI</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Javascript</li>
        </ul>
      </div>
      <div class="col-md-6">
        <h6 class="text-success mb-3">
          <i class="fas fa-infinity"></i> DevOps & CI/CD
        </h6>
        <ul class="list-unstyled ms-3">
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Azure</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Kubernetes</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Docker</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Helm & Kustomization</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Git Action & Argo CD</li>
        </ul>
      </div>
      <div class="col-md-4">
        <h6 class="text-secondary mb-3">
          <i class="fas fa-database"></i> Database & Cache
        </h6>
        <ul class="list-unstyled ms-3">
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>MySQL</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>PostgreSQL</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Redis</li>
        </ul>
      </div>
      <div class="col-md-4">
        <h6 class="text-warning mb-3">
          <i class="fas fa-users"></i> Collaboration
        </h6>
        <ul class="list-unstyled ms-3">
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>JIRA & Confluence</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Git</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Notion</li>
        </ul>
      </div>
      <div class="col-md-4">
        <h6 class="text-danger mb-3">
          <i class="fas fa-chart-line"></i> Monitoring
        </h6>
        <ul class="list-unstyled ms-3">
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Jennifer</li>
          <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i>Azure Monitor</li>
        </ul>
      </div>
    </div>
  </div>
</div>

---

## 🚀 프로젝트

### KT 외국인샵 Azure Migration

<div class="card mb-4 shadow-sm border-primary">
  <div class="card-header bg-white">
    <div class="d-flex justify-content-between align-items-start">
      <div>
        <h5 class="mb-1">Azure 클라우드 환경전환</h5>
        <small class="text-muted">2024.10 ~ 2025.03 | KT DS | DevOps</small>
      </div>
      <span class="badge bg-dark">완료</span>
    </div>
    <div class="mt-2">
      <span class="badge bg-light text-dark border me-1">Kubernetes</span>
      <span class="badge bg-light text-dark border me-1">AKS</span>
      <span class="badge bg-light text-dark border me-1">Azure</span>
      <span class="badge bg-light text-dark border me-1">MySQL</span>
      <span class="badge bg-light text-dark border me-1">App Gateway</span>
      <span class="badge bg-light text-dark border">Helm</span>
    </div>
  </div>
  
  <div class="card-body">
    <div class="mb-3">
      <span class="badge bg-white border text-dark me-2">무중단 전환 성공</span>
      <span class="badge bg-white border text-dark me-2">배포 시간 83% 단축</span>
      <span class="badge bg-white border text-dark me-2">장애 발생률 100% 감소</span>
      <span class="badge bg-white border text-dark">인프라 비용 35% 절감</span>
    </div>
    
    <details>
      <summary class="text-primary" style="cursor: pointer; list-style: none;">
        <strong>📖 상세 내용 보기</strong>
      </summary>
      
      <div class="mt-3 pt-3 border-top">
        <h6 class="fw-bold">💡 프로젝트 배경</h6>
        <p class="small">입사 후 첫 프로젝트로, Docker/Kubernetes 지식이 거의 없는 상태에서 시작했지만 
        해당 프로젝트를 진행하며 클라우드 인프라의 전 과정을 경험할 수 있었습니다.</p>
        
        <h6 class="fw-bold mt-4">🎯 핵심 과제</h6>
        
        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>1. CI/CD 파이프라인 구축</strong></h6>
            <ul class="small mb-0">
              <li>Git Action을 통한 멀티 빌드 환경 구축 (사내 저장소 이용)</li>
              <li>ArgoCD 구성을 통해 안정적인 CD 환경 구축</li>
            </ul>
          </div>
        </div>

        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>2. Kubernetes 환경 구축</strong></h6>
            <ul class="small mb-0">
              <li>Kustomization + Gitops 방식의 yaml 저장</li>
              <li>HA 구성 (replicas: 3, rollingUpdate 전략)</li>
              <li>Pod 리소스 최적화 및 Health Check 설정</li>
            </ul>
          </div>
        </div>

        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>3. 전환에 따른 데이터 동기화</strong></h6>
            <ul class="small mb-0">
              <li>점진적 DNS 전환을 통한 무중단 전환</li>
              <li>DB Data mig를 통해 실시간 데이터 이관</li>
              <li>결과: 데이터 손실 0건</li>
            </ul>
          </div>
        </div>

        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>4. 모니터링 환경 구축</strong></h6>
            <ul class="small mb-0">
              <li>JVM에 Agent 설치를 통한 Azure Monitor 연결</li>
              <li>Alert rule 및 Action Group 설정을 통한 실시간 알림 체계 구축</li>
            </ul>
          </div>
        </div>

        <div class="card mb-3 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>5. 비밀/권한 설정</strong></h6>
            <ul class="small mb-0">
              <li>CI/CD 전과정의 비밀(Git Token, 저장소 Token, Azure Token) Key Vault 연결</li>
              <li>Secret ↔ Azure Key Vault 설정을 통한 소스내 민감정보 관리</li>
              <li>관리 ID/Federation 설정 및 RBAC 설정 관리</li>
            </ul>
          </div>
        </div>
        
        <h6 class="fw-bold mt-4">📊 성과 지표</h6>
        <div class="table-responsive">
          <table class="table table-sm table-bordered">
            <thead class="table-light">
              <tr>
                <th>항목</th>
                <th>AS-IS</th>
                <th>TO-BE</th>
                <th>개선율</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>장애 복구 시간</td>
                <td>60분</td>
                <td>5분</td>
                <td class="text-success fw-bold">92% ↓</td>
              </tr>
              <tr>
                <td>장애 건수</td>
                <td>2회/년</td>
                <td>0회/년</td>
                <td class="text-success fw-bold">100% ↓</td>
              </tr>
              <tr>
                <td>월 인프라 비용</td>
                <td>100만원</td>
                <td>65만원</td>
                <td class="text-success fw-bold">35% ↓</td>
              </tr>
              <tr>
                <td>배포 소요 시간</td>
                <td>15분</td>
                <td>5분</td>
                <td class="text-success fw-bold">67% ↓</td>
              </tr>
            </tbody>
          </table>
        </div>
        
        <div class="alert alert-warning mt-3">
          <strong>🤔 가장 힘들었던 점</strong>
          <p class="mb-0 small">배포 후 특정 pod가 간헐적으로 시작하지 못하는 문제가 있었습니다. 로그를 분석한 결과, 
          pod의 startupProbe가 너무 낮게 설정되어 있던 것이 원인이었습니다. 현재는 자연스럽게 분석이 가능하지만 
          당시에는 k8s에 대한 이해가 거의 전무하던 시절이라 해당 이슈를 잘 이해하지 못하였습니다. 
          그렇기에 당시 <strong>쿠버네티스에 대한 이해의 중요성</strong>을 깨달았고, 
          이후 CKA 시험 준비를 하게 된 계기가 되었습니다.</p>
        </div>
        
        <div class="alert alert-info">
          <strong>😊 느낀점</strong>
          <p class="mb-0 small">입사 직후 던져진 큰 프로젝트였지만, 오히려 <strong>실전에서 배우고 느끼는 것이 가장 빠른 성장</strong>이라는 것을 느꼈습니다. 
          특히 실패의 가치, 문서화의 중요성, 협업의 중요성을 깨달았고, 이 프로젝트 이후 "DevOps 엔지니어"로서의 자신감이 생겼습니다.</p>
        </div>
      </div>
    </details>
    
    <div class="mt-3">
      <a href="https://globalshop.kt.com/global/globalMain.do" class="btn btn-sm btn-outline-primary" target="_blank">
        운영 사이트 →
      </a>
    </div>
  </div>
</div>

### 하이소피 - AI 기반 리뷰 피드백 시스템

<div class="card mb-4 shadow-sm border-success">
  <div class="card-header bg-white">
    <div class="d-flex justify-content-between align-items-start">
      <div>
        <h5 class="mb-1">소상공인을 위한 AI 리뷰 관리 솔루션</h5>
        <small class="text-muted">2025.05 ~ 2025.07 | 사내 교육 MVP (6인) | Backend & DevOps</small>
      </div>
      <span class="badge bg-success">완료</span>
    </div>
    <div class="mt-2">
      <span class="badge bg-light text-dark border me-1">Spring Boot</span>
      <span class="badge bg-light text-dark border me-1">React</span>
      <span class="badge bg-light text-dark border me-1">AKS</span>
      <span class="badge bg-light text-dark border me-1">Event Hub</span>
      <span class="badge bg-light text-dark border me-1">PostgreSQL</span>
      <span class="badge bg-light text-dark border">Redis</span>
    </div>
  </div>
  
  <div class="card-body">
    <details>
      <summary class="text-primary" style="cursor: pointer; list-style: none;">
        <strong>📖 상세 내용 보기</strong>
      </summary>
      
      <div class="mt-3 pt-3 border-top">
        <h6 class="fw-bold">💡 주요 기능</h6>
        <div class="row g-2 mb-3">
          <div class="col-md-6">
            <div class="card bg-light border-0 h-100">
              <div class="card-body py-2">
                <h6 class="small mb-1"><strong>자동 리뷰 수집</strong></h6>
                <p class="small mb-0 text-muted">데이터 크롤링을 통한 다양한 플랫폼 리뷰 통합 관리</p>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div class="card bg-light border-0 h-100">
              <div class="card-body py-2">
                <h6 class="small mb-1"><strong>AI 기반 분석</strong></h6>
                <p class="small mb-0 text-muted">ChatGPT API를 활용한 리뷰 감정 분석 및 인사이트 도출</p>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div class="card bg-light border-0 h-100">
              <div class="card-body py-2">
                <h6 class="small mb-1"><strong>실행계획 생성</strong></h6>
                <p class="small mb-0 text-muted">AI가 추천하는 맞춤형 매장 운영 개선 방안 제공</p>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div class="card bg-light border-0 h-100">
              <div class="card-body py-2">
                <h6 class="small mb-1"><strong>MSA 아키텍처</strong></h6>
                <p class="small mb-0 text-muted">마이크로서비스 기반 확장 가능한 시스템 설계</p>
              </div>
            </div>
          </div>
        </div>
        
        <h6 class="fw-bold mt-4">🎯 기술적 구현</h6>
        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>1. MSA 환경 구축</strong></h6>
            <ul class="small mb-0">
              <li>각 도메인별 독립적인 서비스 분리 (리뷰 수집, 분석, 알림 등)</li>
              <li>Azure Event Hub(Kafka) 기반 비동기 이벤트 통신</li>
              <li>PostgreSQL DB 서비스별 격리</li>
            </ul>
          </div>
        </div>

        <div class="card mb-3 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>2. CI/CD 파이프라인 구축</strong></h6>
            <ul class="small mb-0">
              <li>GitHub Actions를 활용한 자동 빌드</li>
              <li>Argo CD를 통한 GitOps 기반 배포 자동화</li>
              <li>Azure Kubernetes Service에 마이크로서비스 배포</li>
            </ul>
          </div>
        </div>
        
        <div class="alert alert-info">
          <strong>🛠 학습 성과</strong>
          <p class="mb-0 small">MSA 및 클라우드 디자인 패턴에 대한 이론 학습 후, 2주간의 Sprint를 통해 실제 MVP로 구현하며 
          Cloud Native 애플리케이션 개발의 전 과정을 체험했습니다. 이를 바탕으로 실무에서 발생하는 이슈를 
          디자인패턴을 통해 해결할 수 있는 포인트를 모색하고 있습니다.</p>
        </div>
      </div>
    </details>
    
    <div class="mt-3">
      <a href="https://github.com/ktds-garage-04" class="btn btn-sm btn-outline-primary" target="_blank">
        GitHub Repository →
      </a>
    </div>
  </div>
</div>

### 여행지니 - 사진 기반 여행 추천 시스템

<div class="card mb-4 shadow-sm border-secondary">
  <div class="card-header bg-white">
    <div class="d-flex justify-content-between align-items-start">
      <div>
        <h5 class="mb-1">여행을 지니다 + 여행을 추천하다</h5>
        <small class="text-muted">2024.05 ~ 2024.06 | SSAFY (2인) | Web</small>
      </div>
      <span class="badge bg-secondary">완료</span>
    </div>
    <div class="mt-2">
      <span class="badge bg-light text-dark border me-1">Spring Boot</span>
      <span class="badge bg-light text-dark border me-1">Vue.js</span>
      <span class="badge bg-light text-dark border me-1">Docker</span>
      <span class="badge bg-light text-dark border me-1">Argo CD</span>
      <span class="badge bg-light text-dark border">MySQL</span>
    </div>
  </div>
  
  <div class="card-body">
    <details>
      <summary class="text-primary" style="cursor: pointer; list-style: none;">
        <strong>📖 상세 내용 보기</strong>
      </summary>
      
      <div class="mt-3 pt-3 border-top">
        <h6 class="fw-bold">💡 프로젝트 배경</h6>
        <p class="small">SSAFY 과정 중 2인 팀 프로젝트로 시작했으나, 프로젝트 종료 후 <strong>개인 학습 목적</strong>으로 
        기존 서비스를 컨테이너화하고 Azure 클라우드 환경에 CI/CD 파이프라인을 구축했습니다.</p>
        
        <h6 class="fw-bold mt-4">🎯 주요 개선사항</h6>
        <div class="row g-2 mb-3">
          <div class="col-md-6">
            <div class="card bg-light border-0">
              <div class="card-body py-2">
                <h6 class="small mb-1"><strong>기존 프로젝트</strong></h6>
                <ul class="small mb-0">
                  <li>전통적인 모놀리식 아키텍처</li>
                  <li>수동 배포 방식</li>
                </ul>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div class="card bg-light border-0">
              <div class="card-body py-2">
                <h6 class="small mb-1"><strong>개선 후</strong></h6>
                <ul class="small mb-0">
                  <li>Docker 컨테이너화로 환경 일관성 확보</li>
                  <li>Kubernetes 기반 오케스트레이션</li>
                  <li>GitOps 방식의 자동 배포 파이프라인 구축</li>
                </ul>
              </div>
            </div>
          </div>
        </div>
        
        <div class="alert alert-success">
          <strong>🛠 학습 성과</strong>
          <p class="mb-0 small">SSAFY 프로젝트를 단순히 끝내지 않고, 실무에서 사용되는 DevOps 기술을 적용하여 
          클라우드 네이티브 애플리케이션으로 발전시키며 실무 역량 향상에 큰 도움이 되었습니다.</p>
        </div>
      </div>
    </details>
    
    <div class="mt-3">
      <a href="https://github.com/JourneyJinni" class="btn btn-sm btn-outline-primary" target="_blank">
        GitHub Repository →
      </a>
    </div>
  </div>
</div>

### 나주 버스 - iOS 앱

<div class="card mb-4 shadow-sm border-info">
  <div class="card-header bg-white">
    <div class="d-flex justify-content-between align-items-start">
      <div>
        <h5 class="mb-1">나주시 버스 도착 정보 제공 앱</h5>
        <small class="text-muted">2023.03 ~ 현재 | 개인 프로젝트 | iOS App</small>
      </div>
      <span class="badge bg-success">운영중</span>
    </div>
    <div class="mt-2">
      <span class="badge bg-light text-dark border me-1">SwiftUI</span>
      <span class="badge bg-light text-dark border me-1">iOS</span>
      <span class="badge bg-light text-dark border">CoreData</span>
    </div>
  </div>
  
  <div class="card-body">
    <details>
      <summary class="text-primary" style="cursor: pointer; list-style: none;">
        <strong>📖 상세 내용 보기</strong>
      </summary>
      
      <div class="mt-3 pt-3 border-top">
        <h6 class="fw-bold">💡 개발 동기</h6>
        <p class="small">나주시에는 버스 정보를 제공하는 앱이 없어, 공공 API를 활용하여 직접 개발하게 되었습니다.</p>
        
        <h6 class="fw-bold mt-4">🎯 기술적 특징</h6>
        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>서버리스 아키텍처</strong></h6>
            <ul class="small mb-0">
              <li>공공 API 직접 호출로 별도 백엔드 서버 불필요</li>
              <li>CoreData를 활용한 로컬 데이터 캐싱</li>
              <li>운영 비용 0원으로 3년간 서비스 제공</li>
              <li>Google AdMob을 통한 수익화 실현</li>
            </ul>
          </div>
        </div>

        <div class="card mb-3 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>SwiftUI 활용</strong></h6>
            <ul class="small mb-0">
              <li>선언형 UI로 유지보수 용이</li>
              <li>iOS 네이티브 성능 최적화</li>
              <li>다크모드 자동 지원</li>
            </ul>
          </div>
        </div>
        
        <div class="alert alert-success">
          <strong>😊 느낀점</strong>
          <p class="mb-0 small">실제 사용자가 있는 앱을 3년간 운영하면서, 버그 수정, 기능 개선, iOS 버전 업데이트 대응 등 
          앱 개발 실무와 유사한 경험을 쌓을 수 있었습니다.</p>
        </div>
      </div>
    </details>
    
    <div class="mt-3">
      <a href="https://apps.apple.com/kr/app/나주시-버스/id6459411077" class="btn btn-sm btn-outline-primary me-2" target="_blank">
        App Store →
      </a>
      <a href="https://github.com/unggu0704/naju-busInfo" class="btn btn-sm btn-outline-secondary" target="_blank">
        GitHub →
      </a>
    </div>
  </div>
</div>

### BOJ-Extended - 오픈소스 기여

<div class="card mb-4 shadow-sm">
  <div class="card-header bg-white">
    <div class="d-flex justify-content-between align-items-start">
      <div>
        <h5 class="mb-1">백준 크롬 확장 프로그램</h5>
        <small class="text-muted">2023.03 ~ 2023.06 | 교내 오픈소스 프로젝트 (3인)</small>
      </div>
      <span class="badge bg-secondary">완료</span>
    </div>
    <div class="mt-2">
      <span class="badge bg-light text-dark border me-1">JavaScript</span>
      <span class="badge bg-light text-dark border">Chrome API</span>
    </div>
  </div>
  
  <div class="card-body">
    <details>
      <summary class="text-primary" style="cursor: pointer; list-style: none;">
        <strong>📖 상세 내용 보기</strong>
      </summary>
      
      <div class="mt-3 pt-3 border-top">
        <h6 class="fw-bold">💡 기여 내용</h6>
        <p class="small">알고리즘 문제를 풀 때 시간 측정이 필요한 사용자를 위해 <strong>타이머 기능</strong>을 추가하는 오픈소스 기여 프로젝트</p>
        
        <h6 class="fw-bold mt-4">🎯 학습 성과</h6>
        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>오픈소스 협업 경험</strong></h6>
            <p class="small mb-0 text-muted">Git Flow, PR, 코드 리뷰 프로세스 학습</p>
          </div>
        </div>

        <div class="card mb-2 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>Chrome Extension API</strong></h6>
            <p class="small mb-0 text-muted">브라우저 확장 프로그램 개발 경험</p>
          </div>
        </div>

        <div class="card mb-3 bg-light border-0">
          <div class="card-body py-2">
            <h6 class="small mb-1"><strong>JavaScript 심화</strong></h6>
            <p class="small mb-0 text-muted">DOM 조작, 이벤트 처리, 로컬 스토리지 활용</p>
          </div>
        </div>
      </div>
    </details>
    
    <div class="mt-3">
      <a href="https://github.com/unggu0704/boj-extended" class="btn btn-sm btn-outline-primary" target="_blank">
        GitHub Repository →
      </a>
    </div>
  </div>
</div>

---

## 📝 기술 블로그

<div class="list-group mb-3">
{% for post in site.posts limit:5 %}
  <a href="{{ post.url | relative_url }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-start">
    <div class="ms-2 me-auto">
      <div class="fw-bold">{{ post.title }}</div>
    </div>
    <span class="badge bg-secondary rounded-pill">{{ post.date | date: "%Y.%m.%d" }}</span>
  </a>
{% endfor %}
</div>

<a href="/archives/" class="btn btn-outline-secondary">
  <i class="fas fa-archive"></i> 전체 포스트 보기
</a>


## 📞 Contact

<div class="text-center mb-4">
  <div class="row g-3 justify-content-center">
    <div class="col-6 col-md-3">
      <a href="mailto:kyuhyeong.kim@kt.com" class="text-decoration-none d-block">
        <div class="card h-100 border-primary shadow-sm hover-shadow">
          <div class="card-body text-center py-4">
            <i class="fas fa-envelope fa-2x text-primary mb-2"></i>
            <p class="mb-0 small fw-bold">Email</p>
          </div>
        </div>
      </a>
    </div>
    <div class="col-6 col-md-3">
      <a href="https://github.com/unggu0704" class="text-decoration-none d-block" target="_blank">
        <div class="card h-100 border-dark shadow-sm hover-shadow">
          <div class="card-body text-center py-4">
            <i class="fab fa-github fa-2x text-dark mb-2"></i>
            <p class="mb-0 small fw-bold">GitHub</p>
          </div>
        </div>
      </a>
    </div>
    <div class="col-6 col-md-3">
      <a href="https://www.linkedin.com/in/규형-김-5b5b10299" class="text-decoration-none d-block" target="_blank">
        <div class="card h-100 border-info shadow-sm hover-shadow">
          <div class="card-body text-center py-4">
            <i class="fab fa-linkedin fa-2x text-info mb-2"></i>
            <p class="mb-0 small fw-bold">LinkedIn</p>
          </div>
        </div>
      </a>
    </div>
    <div class="col-6 col-md-3">
      <a href="https://unggu.xyz" class="text-decoration-none d-block" target="_blank">
        <div class="card h-100 border-success shadow-sm hover-shadow">
          <div class="card-body text-center py-4">
            <i class="fas fa-blog fa-2x text-success mb-2"></i>
            <p class="mb-0 small fw-bold">Blog</p>
          </div>
        </div>
      </a>
    </div>
  </div>
</div>

<style>
.hover-shadow:hover {
  transform: translateY(-5px);
  transition: all 0.3s ease;
  box-shadow: 0 0.5rem 1rem rgba(0, 0, 0, 0.15) !important;
}
</style>

---

> 함께 성장할 수 있는 기회가 있다면 언제든 연락 주세요! 😊
{: .prompt-tip }
